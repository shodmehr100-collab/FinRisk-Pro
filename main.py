...
DEMO_CASES = {
    "1": {
        ...
        "mu": 0.25,        # Агрессивный рост
        "sigma": 0.45,     # Огромная волатильность рынка
        "operating_cost": 15000
    },
    "2": {
        "name": "Retail Shop (Средний риск)",
        "initial_capital": 50000,
        "mu": 0.08,        # Умеренный рост
        "sigma": 0.18,     # Средняя стабильность
        "operating_cost": 5000
    },
    "3": {
        "name": "Manufacturing Plant (Низкий риск)",
        "initial_capital": 500000,
        "mu": 0.04,        # Медленный рост
        "sigma": 0.09,     # Высокая стабильность производств
...
# ==============================================================================
def init_db():
    """Создание инфраструктуры хранения проектов, если она отсутствует"""
...
# Инъекция кастомных стилей для интерфейса
st.markdown("""
    <style>
...
def get_roi_warning(roi):
    """Критический комплаенс-контроль аномалий финансовой модели"""
    if roi > 300:
...
def normalize_risk_roi(roi, target=25, max_sustainable=80):
    """Защита от аномалий: штрафует скоринг за нереалистичный ROI"""
    if roi < 0: return 100  
...
def calculate_risk_score(bankruptcy_prob, var_percent, margin_ratio, roi):
    """Интегральный квантильный скоринг надежности проекта"""
...
# ==============================================================================
# 3. МАТЕМАТИЧЕСКОЕ И АНАЛИТИЧЕСКОЕ ЯДРО (МОНТЕ-КАРЛО С ИСПРАВЛЕНИЯМИ)
# ==============================================================================
@st.cache_data(ttl=3600)
def run_monte_carlo_institutional(data_dict, scenario, num_simulations=1000, months=12, demand_multiplier=1.0):
    """
    Симуляция Монте-Карло уровня Quant-Finance:
    - Моделирование спроса через Geometric Brownian Motion (GBM)
    - Исправлено: Начальный баланс = -capital (CAPEX отток)
    - Исправлено: Логика банкротства триггерится строго 1 раз за траекторию
    - Исправлено: Отрицательные остатки НЕ зануляются для корректного расчета VaR/ES
    """
    ...
    for _ in range(num_simulations):
        # Ошибка 1 ПОФИКСЕНА: Инвестиции — это первоначальный отток (CAPEX)
        balance = -capital 
        path = []
        is_bankrupt = False
        ...
        for m in range(1, months + 1):
            ...
            sim_margin = np.clip(sim_margin, margin_per_unit * 0.1, margin_per_unit * 3.0)
            
            monthly_cf = (sim_sales * sim_margin) - fixed
            balance += monthly_cf
            
            # Ошибка 2 ПОФИКСЕНА: Проверка дефолта строго 1 раз на симуляцию
        ...
        # Ошибка 2 ПОФИКСЕНА: Сохраняем реальный финансовый итог (включая жесткие минусы) для квантилей рисков
        final_balances.append(balance)
            
    # Ошибка 3 ПОФИКСЕНА: Настоящий расчет чистых потерь (Net Losses)
    # Потери = Чистый Первоначальный Капитал - Конечный Баланс. 
    # Если Конечный Баланс отрицательный, то потери превышают стартовый капитал.
    final_balances_np = np.array(final_balances)
    losses = capital - final_balances_np  
    
    worst_5_percent_loss = np.percentile(losses, 95) # 95-й перцентиль максимальных убытков
...
def generate_forecast_metrics(data, scenario, demand_multiplier=1.0, fast_mode=False):
    ...
    avg_final_profit = np.mean(mc_results['final_balances'])
    capital = data['start_capital']
    # ROI на основе чистого изменения стоимости
    roi = ((avg_final_profit - (-capital)) / capital) * 100 if capital > 0 else 0
...
# ==============================================================================
# ВКЛАДКА 1: АНАЛИТИЧЕСКИЙ ДАШБОРД
# ==============================================================================
with tab_dash:
    # Вычисляем статус вердикта на основе жестких математических порогов квантилей
    ...
    # Главный баннер-вердикт для жюри (Вау-эффект за 3 секунды)
    st.markdown(f"""
    ...
    # Большие презентабельные карточки (Метрики верхнего уровня)
    ...
    # Ниже пошёл твой радарный график
...
# ==============================================================================
# ВКЛАДКА 2: МОДЕЛИРОВАНИЕ МОНТЕ-КАРЛО (ИСПРАВЛЕННЫЙ BLOOMBERG CONE)
# ==============================================================================
with tab_monte:
    ...
    # Расчет квантилей БЕЗ ОШИБКИ ВЫЖИВШЕГО через np.nanpercentile
